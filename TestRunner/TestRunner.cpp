...
static std::unique_ptr<llvm::Module> parseIRFromFile(LLVMContext &Ctx,
  ...
  if (!TheModule) {
    ...
    Err.print("clang-O3-challenge", errs());
    return nullptr;
  }
  return TheModule;
}
...
std::unique_ptr<BenchmarkBase> TestRunner::createBenchmark(StringRef Name,
    FunctionStorage &Storage) {
  if (Name == "sink")
    return std::make_unique<SinkBenchmark>(Storage);
  if (Name == "sum-1")
    return std::make_unique<Sum1Benchmark>(Storage);
  if (Name == "sum-2")
    return std::make_unique<Sum2Benchmark>(Storage);
  if (Name == "sum-3")
    return std::make_unique<Sum3Benchmark>(Storage);
  if (Name == "sum-4")
    return std::make_unique<Sum4Benchmark>(Storage);
  return nullptr;
}
...
bool TestRunner::runBenchmark(std::filesystem::path BenchDir) {
  ...
  FunctionStorage Storage(std::move(BenchModule), std::move(Context), *JIT);
  auto Bench = createBenchmark(BenchName, Storage);
  assert(Bench && "Must be known benchmark");

  BenchRunner Runner(*JIT, *Bench);
