...
Основное задание - оптимизировать набор бенчмарок `sum-*` как можно лучше.  
...
Предлагается использовать существующие LLVM-ные пассы. Неполный список с краткими описаниями пассов см. [здесь](https://llvm.org/docs/Passes.html).  
При использовании пасса нужно уметь объяснить, зачем он нужен в конкретном примере (бенчмарке) и что он делает сам по себе.  
Полный список всех доступных LLVM пассов можно найти в `llvm-project/llvm/lib/Passes/PassRegistry.def`.  

Для выяснения, каких же пассов не хватает в вашем пайплайне, можно подсматривать в то, что делает llvm opt при запуске с O3 - какие пассы запускаются и какие изменения они делают в IR-е.  
Подробнее об этом см. в описании `sum-1` ниже.  

Разные бенчмарки направлены на проверку разных оптимизаций, но между ними есть пересечения. Т.е. при улучшении одной бенчмарки вполне вероятно улучшатся остальные.  

Вывод алгоритмов в бенчмарках проверяется с референсным, поэтому некорректные оптимизации не сработают.  

## Бенчмарки

- `sum-1` - сумма двух массивов. Нужно оптимизировать функцию `sum_1`.  
  Основная оптимизация здесь - векторизация. Для того, чтобы она смогла отработать, необходимо упростить цикл.  
  ...
  $ clang-O3-challenge/generate-cfgs.sh /path/to/llvm/build/bin/opt sum-1.opt-O3.ll
  ```
  _Note: запускайте `opt` именно на .ll.init файле. На sum-1.ll не получится - в нём нет информации о вашем CPU. В init она добавляется автоматически._   
